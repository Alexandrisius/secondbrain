...
// - Мы ХРАНИМ в ноде ТОЛЬКО метаданные и `attachmentId` (теперь это `docId` из библиотеки),
//   а сами файлы лежат на диске в глобальной библиотеке документов:
...
export interface NeuroNodeData {
  ...
  // =============================================================================
  // DEMO MODE META (server-selected)
  // =============================================================================
  //
  // Зачем это нужно:
  // - В demo режиме сервер может:
  //   - подставлять demo apiKey,
  //   - выбирать модель динамически (любая `:free`),
  //   - отключать image input.
  // - UI должен честно и наглядно сообщать пользователю:
  //   - что активен demo режим,
  //   - какая модель реально использовалась,
  //   - почему изображения не учитывались.
  //
  // ВАЖНО:
  // - Эти поля — НЕ "настройки", а метаданные ПОСЛЕДНЕГО запроса генерации.
  // - Они заполняются клиентом на основе response headers от /api/chat.

  /**
   * Флаг: последняя генерация выполнялась в Demo Mode.
   *
   * null/undefined:
   * - означает “ещё не было запроса” или “старые данные без этого поля”.
   */
  demoMode?: boolean;

  /**
   * Какая модель реально использовалась на последней генерации.
   *
   * Важно:
   * - в demo режиме это будет авто-выбранная `:free` модель,
   * - в обычном режиме — то, что выбрал пользователь.
   */
  modelUsed?: string | null;

  /**
   * Флаг: в demo режиме изображения были проигнорированы сервером (не отправлены в LLM).
   *
   * Это нужно для UX (честная подсказка рядом с вложениями).
   */
  demoIgnoredImages?: boolean;
  ...
  //
  // Как реализуем в MVP:
  // - Сами файлы остаются на диске (data/library/**).
